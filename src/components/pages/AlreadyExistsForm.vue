...
<template>
  <div class="bg-blue-50 dark:bg-gray-800 w-[100%] sm:w-[90%] rounded-xl pt-15 pb-5 px-5 relative">
    <div
    ...
    <div class="p-5 bg-white rounded-lg shadow-md dark:bg-gray-700 dark:text-white">
      <div class="mb-5 relative">
        ...
        <div class="relative">
          ...
          <input v-model="studentStore.studentId" @blur="studentStore.fetchCourses" :disabled="studentStore.otpSent"
            type="text" id="name-input" placeholder="Enter Your ID"
      ...
      <div class="mb-5" v-if="!showAllCourses && studentStore.courses.data">
        <label class="block text-sm font-medium dark:text-gray-300 text-gray-900">
          Module Name:
        </label>
        <div class="flex items-center gap-2">
          ...
          <button @click="showAllCourses = true"
      ...
      <div class="mb-5" v-if="showAllCourses">
        <label class="block text-sm font-medium dark:text-gray-300 text-gray-900">
        ...
        <div class="flex items-center gap-2">
          ...
          <button @click="showAllCourses = false" class="text-sm text-gray-600 hover:underline">
            Back
          </button>
        </div>
      </div>
      ...
      <div class="mb-5" v-show="studentStore.instructors.data">
        <label for="instructor-select"
          class="block dark:text-gray-300 text-sm font-medium text-gray-900">Instructor:</label>
        <select v-model="studentStore.selectedInstructor" id="instructor-select" class="input-field">
          ...
          <option class="dark:bg-gray-800" v-for="instructor in studentStore.instructors.data" :key="instructor.id"
            :value="instructor.id">
            {{ instructor.name }}
          </option>
        </select>
      ...
      <div class="mb-5 relative" v-if="
        !studentStore.otpSent &&
        studentStore.selectedModule &&
        studentStore.selectedInstructor
      ">
        <div class="flex justify-center">
          <button @click="handelSendOtp" :disabled="(studentStore.timer < 120 && studentStore.timer > 0) ||
            studentStore.otpSent ||
            studentStore.studentId === ''
            " class="text-primary absolute top-4 z-10 right-3 cursor-pointer">
            <span v-if="studentStore.loadingOtp"><i class="fa-solid fa-circle-notch fa-spin-pulse"></i></span>
            <span v-else :class="{
        ...
        <div class="relative">
          <!-- OTP Input -->
          <input type="text" v-model="studentStore.studentOTP"
            :placeholder="`It will be sent after ${studentStore.timer} seconds`" :disabled="studentStore.otpSent ||
              studentStore.studentId === '' ||

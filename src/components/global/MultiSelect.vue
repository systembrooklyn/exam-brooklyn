<template>
  <div class="relative" ref="container">
    ...
    <div
      ...
      :class="{ 'ring-2 ring-indigo-500/20 border-indigo-500': isOpen }" @click="toggleDropdown">
      <!-- Cap height so many tags don’t break modals / layout; scroll inside. -->
      <div class="max-h-52 min-h-[42px] overflow-y-auto overflow-x-hidden p-1.5 pr-10 scroll-smooth">
        <div class="flex flex-wrap gap-1.5 content-start">
          ...
          <div v-for="item in selectedOptions" :key="item[valueKey]"
            ...
            <button type="button" @click.stop="removeItem(item)"
              class="hover:text-indigo-900 focus:outline-none shrink-0" :aria-label="`Remove ${getOptionLabel(item)}`">
    ...
    <div v-if="isOpen"
      ...
      <div class="p-2 sticky top-0 bg-white border-b border-gray-100 z-[1]">
        <input ref="searchInput" v-model="searchQuery" type="text" placeholder="Search..."
          ...
          @click.stop />
        <button v-if="showSelectAll && options.length" type="button"
          ...
          <div class="w-4 h-4 rounded border flex items-center justify-center transition-colors duration-150 shrink-0"
            :class="selectAllBoxClass">
            <svg v-if="selectAllFilteredState === 'all'" class="w-3 h-3 text-white" fill="none" stroke="currentColor"
              viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" />
            </svg>
            <svg v-else-if="selectAllFilteredState === 'some'" class="w-3 h-3 text-indigo-700" fill="none"
              stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div class="py-1">
        <div v-for="option in filteredOptions" :key="option[valueKey]"
          class="px-3 py-2 hover:bg-gray-50 cursor-pointer flex items-center gap-2" @click.stop="toggleOption(option)">
          <div class="w-4 h-4 rounded border flex items-center justify-center transition-colors duration-150"
            :class="isSelected(option) ? 'bg-indigo-600 border-indigo-600' : 'border-gray-300 bg-white'">
            <svg v-if="isSelected(option)" class="w-3 h-3 text-white" fill="none" stroke="currentColor"
              viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path>
            </svg>
          </div>
          <div class="flex flex-col text-left">
            <span class="text-sm text-gray-700" :class="{ 'font-medium text-indigo-700': isSelected(option) }">{{
              getOptionLabel(option) }}</span>
            <span v-if="subLabelKey && option[subLabelKey]" class="text-xs text-gray-400 mt-0.5">{{ option[subLabelKey]
              }}</span>
          </div>
        </div>

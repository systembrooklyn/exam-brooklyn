...
export const usePlacementTestsExamStore = defineStore("placementTestsExam", () => {
...
const examAnswers = ref([]);
  async function fetchPlacementExam(testId, studentId) {
    ...
    try {
      ...
      Cookies.set('attempt_id', response.data.data.attempt_id, { expires: 7 });
...
async function submitFinalExam(answers) {
  ...
  try {
    const response = await apiClient.post(`${FINISH_PLACEMENT}/${attemptId}/submit`, {
      answers: formattedAnswers,
    });
    console.log("Final exam submitted successfully:", response.data);
    notyf.success( response.data.message ||"Final exam submitted successfully!");
  } catch (error) {

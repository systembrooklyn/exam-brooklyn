  ...
  <div class="bg-white rounded-2xl shadow-sm p-6 animate-fade-in min-h-[400px]">
    ...
    <div v-else class="space-y-4">
      ...
      <div v-else-if="payrollData" class="space-y-5 border border-indigo-100 rounded-2xl p-4">
        ...
        <div
          v-if="payrollData.other_deductions?.items?.length"
          class="space-y-3"
        >
          <div class="bg-slate-100 text-slate-700 font-bold text-sm py-2 px-4 rounded-lg flex items-center justify-between">
            ...
            <span class="inline-flex items-center rounded-full bg-red-100 px-2.5 py-0.5 text-xs font-bold text-red-700 tabular-nums">
              {{ formatMoney(payrollData.other_deductions.total) }} EGP
            </span>
          </div>
          ...
          <div class="overflow-x-auto border border-gray-200 rounded-xl">
            <table class="w-full text-sm min-w-[600px]">
              ...
              <tbody class="divide-y divide-gray-100">
                <tr
                  v-for="(ded, index) in payrollData.other_deductions.items"
                  :key="ded.id"
                ...
                >
                  ...
                  <td class="px-4 py-3 text-gray-500 align-top">
                    <NotesWithLinks v-if="ded.notes" :text="ded.notes" />
                    <span v-else>-</span>
                  </td>
                </tr>
                <!-- Total row -->
                <tr class="bg-red-50/60 border-t-2 border-red-200">
                  ...
                  <td class="px-4 py-3 text-red-700 font-bold tabular-nums">
                    {{ formatMoney(payrollData.other_deductions.total) }}
                  </td>
                  <td colspan="2" />
                </tr>
              </tbody>
